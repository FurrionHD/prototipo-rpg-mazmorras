...
func aplicar_roster(roster: Dictionary) -> void:
	...
	altas._recomponer_fila_enemigos()
	_update_hp()
...
func _valores(lista: Array) -> Array:
	...
	for c in lista:
		...
		out.append([c.current_hp, c.current_mp, c.current_energy, efectos._chips_de(c),
...
func aplicar_impactos(datos: PackedInt32Array) -> void:
	...
	while j + 4 < datos.size():
		...
		if victima == null:
			continue
		efectos._fx_tanda(maxi(0, tanda))
		...
		efectos._fx_golpe(_de_codigo(ca), victima, dmg, (flags & 1) != 0, (flags & 2) != 0,
...
func _apagar_caidos() -> void:
	for i in _bloques.size():
		if i >= _enemies.size() or _enemies[i].is_alive():
			continue
		altas._apagar_bloque(_enemies[i])
		if _timeline != null:
			_timeline.quitar(_enemies[i])
	for i in _bloques_aliados.size():
		if i >= _aliados.size() or _aliados[i].is_alive():
			continue
		altas._apagar_diferido(_bloques_aliados[i], true)
		if _timeline != null:
...
func _setup_ui() -> void:
	...
	for i in _aliados.size():
		altas._anadir_bloque_aliado(_aliados[i])

...
func _enemy_turn(e: Combatant) -> void:
	...
	msg += magia._elem_golpe_txt(result, e.elemento_ataque)
	if bool(_defendiendo.get(obj, false)):
		msg += " (defendido 🛡️)"
...
func _enemy_use_ability(e: Combatant, ab: AbilityData, victima: Combatant = null) -> void:
	...
	if ab.dano_mult > 0.0:
		...
		msg = magia._log_desglose(titulo, rastro, tocados, dano_por_obj, total, sin_dar)
	else:
	...
	msg += magia._elem_reparto_txt(e.elemento_ataque, mult_por_obj)

...
func aplicar_roster(roster: Dictionary) -> void:
	...
	for i in filas.size():
		...
		if i < _enemies.size():
			...
			_enemies.append(c)
			var b: Dictionary = figuras._crear_bloque(c, i + 1, i)
			_bloques.append(b)
...
func retomar(estado: Dictionary, cs: Array, filas_e: Array) -> void:
	...
	# no existia.
	objetivos._reenlazar_coberturas()
	...
	match String(accion.get("tipo", "atacar")):
		...
		"habilidad":
			...
			if elegida != null:
				...
				habilidades._usar_habilidad(elegida)
			else:
		...
		"magia":
			...
			if hechizo != null:
				...
				magia._elegir_hechizo(hechizo, al_m)
			else:
		...
		"frase":
			...
			if _cast_spell == null or _cast_index >= _cast_spell.longitud():
				return
			magia._responder_frase(String(accion.get("texto", "")), _cast_spell.frases[_cast_index])
		"cancelar":
		...
		"objeto":
			...
			if cons != null:
				objetos._usar_objeto(cons, al, false)   # ya la pago el de su bolsa, aqui solo se resuelve
			else:
				_accion_atacar()
		_:
...
func _setup_ui() -> void:
	...
	for i in _enemies.size():
		var b: Dictionary = figuras._crear_bloque(_enemies[i], i + 1, i)
		_bloques.append(b)
		_bloques_box.add_child(b["columna"])
...
func _anadir_bloque_aliado(c: Combatant) -> void:
	var ba: Dictionary = figuras._crear_bloque(c, 0, -1)
...
func _apagar_visual(b: Dictionary, es_aliado: bool) -> void:
	...
	var nodo_muerto: Node = figuras._nodo_pose_de(b)
	if nodo_muerto != null:
		figuras._pose_marcar(nodo_muerto, figuras.PoseSprite.MUERTE)
	...
	var muriendo: bool = figuras._arrancar_muerte(b, nodo_muerto)
	...
	if not es_aliado:
		...
		if cur != null and is_instance_valid(cur):
			cur.visible = false
	panel.add_theme_stylebox_override("panel", figuras._sb_bloque(false))
...
func _meter_enemigo(c: Combatant, es_invocado: bool, hueco: int = -1) -> int:
	...
	if idx >= 0:
		...
		_enemies.append(c)
		var b: Dictionary = figuras._crear_bloque(c, idx + 1, idx)
		_bloques.append(b)
		_bloques_box.add_child(b["columna"])
...
func _revivir_bloque(i: int, c: Combatant) -> void:
	...
	if fig != null and is_instance_valid(fig):
		...
		if act != null:
			figuras._marcar_mutante(act, fig, c)
...
func _chip(box: Container, texto: String, tooltip: String, idx: int = -1,
	...
	var clic: Callable = figuras._seleccionar.bind(idx) if idx >= 0 else Callable()
...
func _montar_log() -> void:
	...
	_boton_detalle.pressed.connect(figuras._abrir_detalle.bind(null))

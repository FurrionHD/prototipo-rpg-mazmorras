...
func plantar(t: float) -> void:
	...
	# EL SONIDO TAMBIEN CUANDO EL RELOJ VIENE DE FUERA. El visor apaga el _process de aqui y lleva el
	# tiempo el mismo, asi que sin esto la animacion se ve muda justo en la herramienta que existe
	# para juzgar su ritmo -- y el sonido es la mitad del ritmo.
	#
	# Retroceder no vuelve a disparar nada (los hitos solo avanzan), y saltar hacia delante suelta de
	# golpe los que se hayan cruzado, que es lo razonable para una herramienta.
	_sonar_si_toca()
	if _mu != null and is_instance_valid(_mu):
		_colocar_maestro()
	if _fondo != null:

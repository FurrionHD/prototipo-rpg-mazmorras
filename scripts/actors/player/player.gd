...
func _enemigos_a_tiro(alcance: float = -1.0) -> Array:
	...
	for e in get_tree().get_nodes_in_group("enemy"):
		...
		# medios cuerpos para poder compararlos. Sale de la caja y no de un 32 escrito a mano
		...
		if hueco > rango - (PoseJugador.CAJA_CUERPO.size.x * 0.5 + Cuerpos.MEDIO_BASE):
			continue
...
#
# Va como METODO y no como constante porque es asi como lo pregunta Cuerpos.caja_de (por
# has_method): el bicho no tiene que saber si lo que tiene delante es el jugador, un compañero o el
# otro humano, solo si sabe decir cuanto ocupa. Lo que no lo sepa se queda con el cuerpo base de
# 32x32.
...
#
# Delega en Cuerpos, que es EL MISMO sitio del que sale el hueco cuando lo pregunta el bicho. Aqui
# habia una copia de la formula y ya se habia separado de la otra (esta llevaba un 32 escrito a mano
# donde aquella usaba su constante). Con la caja del jugador desplazada eso deja de ser un detalle:
# la cuenta es un AABB contra AABB y dos versiones de ella se van a desincronizar seguro.
func _hueco_hasta(otro: Node) -> float:
	if not (otro is Node2D):
		return INF
	return Cuerpos.hueco(self, otro as Node2D)

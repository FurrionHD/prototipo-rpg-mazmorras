...
func poner_cara(tex: Texture2D) -> void:
	# SIN IMAGEN NO SE PONE NADA AQUI, y eso ya no deja la cabeza pelada: los ojos y la boca son una
	# CAPA HORNEADA como el resto (ver cara_sprites.gd), que se monta justo cuando no hay foto.
	#
	# Hubo en medio una version con un circulo liso de color carne. Cumplia lo minimo -- que la cabeza
	# no fuera una bola de pelo -- pero se leia como una pegatina, que es lo que pasa cuando lo unico
	# que separa la cara de la cabeza es un reborde difuminado. Una cara son DOS OJOS.
	if tex == null:
		if _cara != null:
			_cara.queue_free()
			_cara = null
...
func _ordenar() -> void:
	...
	for i in _capas.size():
		...
		# CAPA CON Z PROPIO: no entra en el reparto.
		#
		# LO LLEVAN TODAS LAS QUE VISTEN EL EJE DEL CUERPO -- el propio cuerpo, la ropa, el pelo y la
		# cara --, y esto costo un fallo que solo se veia ANDANDO: la camisa desaparecia de espaldas.
		# El ancla del torso esta en x=0, y=0, asi que su profundidad es cero... hasta que la pose
		# INCLINA el tronco sobre la cadera y lo mueve dos decimas. De espaldas esas dos decimas son
		# negativas, y 'round(-0,88) x 16' las convierte en z = -14: la camisa se iba DEBAJO del
		# cuerpo y este la tapaba entera. Quieto no pasaba porque el idle inclina cuatro veces menos
		# y el redondeo lo devolvia a cero.
		#
		# La regla que sale de ahi: si una pieza va SIEMPRE delante o SIEMPRE detras, su z se escribe;
		# la formula de abajo es solo para lo que de verdad cruza al otro lado del cuerpo al girar
		# (una mano, un escudo), que son piezas bien separadas del eje.
		#
